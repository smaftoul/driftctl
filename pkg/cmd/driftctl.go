...
func NewDriftctlCmd(build build.BuildInterface) *DriftctlCmd {
	...
	if cmd.build.IsThirdPartyNetworkCallsAllowed() {
...
func (driftctlCmd DriftctlCmd) ShouldCheckVersion() bool {
	...
	return driftctlCmd.build.IsRelease() && driftctlCmd.build.IsThirdPartyNetworkCallsAllowed() && !hasVersionCmd && !hasCompletionCmd && !noVersionCheckVal && !isHelp && !noVersionCheckEnv
}
